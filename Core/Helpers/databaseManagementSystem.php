...
class DatabaseManagementSystem extends Model
{
    ...
    protected function addUserRole(): void
    {
        ...
        foreach ($roles as $role) {
            if (!in_array($role, $roleName)) {
                $sql = "INSERT INTO `userRole` (`roleName`) VALUES (?)";
                $this->query($sql, [$role]);
            }
        }
    }
    ...
    {
        $urls = [
            ['url' => 'home', 'persianUrl' => 'خانه', 'dropdown' => '0', 'parentId' => 0, 'sort' => 1],
            ['url' => 'products', 'persianUrl' => 'محصولات', 'dropdown' => '1', 'parentId' => 0, 'sort' => 2],
            ['url' => 'product1', 'persianUrl' => 'محصول1', 'dropdown' => '0', 'parentId' => 2, 'sort' => 1],
        ];
        foreach ($urls as $url) {
            $sql = "SELECT * FROM `url` WHERE `url` <=> '$url[url]'";
            if (!$this->connection->query($sql)->num_rows) {
                Helper::getArrayKeysAsString($url, $columns);
                Helper::getArrayValuesAsString($url, $values);
                $sql = "INSERT INTO `url` ($columns) VALUES ($values)";
                $this->query($sql, array_values($url));
            }
        }
    }
    ...
    public function addContent(): void
    {
        ...
        $sql = "SELECT `title`,`content` FROM `content`";
        if (!$this->connection->query($sql)->num_rows) {
            foreach ($content as $post) {
                Helper::getArrayKeysAsString($post, $columns);
                Helper::getArrayValuesAsString($post, $values);
                $sql = "INSERT INTO `content` ($columns) VALUES ($values)";
                $this->query($sql, array_values($post));
            }
        }
    }
    ...
    {
        $urls = [
            ['url' => 'api/user/dashboard', 'persianUrl' => 'داشبورد', 'dropdown' => '0', 'parentId' => 0, 'sort' => 1],
            ['url' => 'ticket', 'persianUrl' => 'تیکت پشنیبانی', 'dropdown' => '1', 'parentId' => 0, 'sort' => 2],
            ['url' => 'api/ticket/create', 'persianUrl' => 'ایجاد تیکت', 'dropdown' => '0', 'parentId' => 2, 'sort' => 1],
            ['url' => 'api/ticket/index', 'persianUrl' => 'تیکت های ارسال شده', 'dropdown' => '0', 'parentId' => 2, 'sort' => 2],
            ['url' => 'api/user/logout', 'persianUrl' => 'خروج از حساب کلربری', 'dropdown' => '0', 'parentId' => 0, 'sort' => 3],
        ];
        ...
        foreach ($urls as $url) {
            ...
            if (!$this->connection->query($sql)->num_rows) {
                Helper::getArrayKeysAsString($url, $columns);
                Helper::getArrayValuesAsString($url, $values);
                $sql = "INSERT INTO `userPanelUrl` ($columns) VALUES ($values)";
                $this->query($sql, array_values($url));
            }
        }
    }
    ...
    {
        $urls = [
            ['url' => 'api/panel/dashboard', 'persianUrl' => 'داشبورد', 'sort' => 1],
            ['url' => 'api/ticket/received', 'persianUrl' => 'تیکت های دریافتی', 'sort' => 2],
            ['url' => 'api/panel/logout', 'persianUrl' => 'خروج از حساب کلربری', 'sort' => 3],
        ];
        ...
        foreach ($urls as $url) {
            ...
            if (!$this->connection->query($sql)->num_rows) {
                Helper::getArrayKeysAsString($url, $columns);
                Helper::getArrayValuesAsString($url, $values);
                $sql = "INSERT INTO `adminPanelUrl` ($columns) VALUES ($values)";
                $this->query($sql, array_values($url));
            }
        }
    }
