...
class Router extends Route
{
    ...
    // {
    //     $token = Token::fetch();
    //     $csrfToken = md5(strrev(md5(Helper::userIp() . ($token->iat * $token->exp))));
    //     if (!hash_equals($token->data->csrf, $csrfToken))
    //         Helper::invalidToken();
    //     return $this;
    // }
    ...
    {
        $user = Token::fetch()->data;
        if (!isset($user->userId, $user->roleId))
            Helper::invalidToken();
        return $this;
    }
